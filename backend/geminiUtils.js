...
async function callGeminiForFinalResponse(userInput, toolSelectionResponse, toolOutputs, userProfile, functionCalls) {
  ...
  const systemInstruction = {
    ...
    parts: [{
      ...
      2.  **사용자의 '일정(Schedule)' 정보를 확인할 때, 반드시 '요청 날짜'와 일정에 명시된 날짜를 정확히 비교해줘.** 일정 날짜와 요청 날짜가 같거나 가까운 경우에만 언급하고, 날짜 관계를 정확히 표현해줘. 예: "오늘(12/16) '성수 카페 탐방' 일정이 있으시네요!" 또는 "내일(12/17) '마라톤' 일정이 있으시네요!" 날짜가 멀리 떨어져 있다면 (예: 오늘이 12/2인데 12/17 일정) 그 일정은 언급하지 마.
      ...
      **[핵심 원칙]**
      ...
      - 사용자의 일정 날짜가 요청 날짜와 같거나 가까우면 반드시 언급
